/*
    ...
    2015.6.30.00:23     So I tried a branch of this code, DBA2eTwoGapFlags, which was a version that would subtract 8 if a gap started
                        in either of the words, instead of counting gaps in both words as part of the same gap.  It gave me the same
                        answers for the test problems, as did the code before I tried this, but it also failed the evaluation.
                        I've tried both of these options with printing out the highest value in the grid instead of the last value,
                        but that fails the test questions.  I've tried not printing a newline after the end of output (which was the stupid reason
                        that my code was only partially accepted one time) but that made no difference.  I've no idea why it's no good.
                        So yeah.  I'm really out of ideas now.  I think it works, unless there's something in the problem I don't get.

                        I think I might be able to use this code for another problem on the site, so that's next.
                        I probably accidentally got partial credit for one of my submissions of this code, so that's better than nothing.

    This copy is the same as dnafinal.  Use it for messing with debug and shit.
